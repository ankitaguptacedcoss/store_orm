...
$category = $_REQUEST['id']; //getting category of product
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
  ...
  foreach ($cloth as $k => $v) {
    ...
    echo "<form method='post' action='../controller/deleteProduct.php'><button type='submit' class='btn btn-outline-primary' value='$v->product_id' name='delete'>Delete Product</button></form><br>";
    echo "<form method='post' action='../controller/editProduct.php'><button type='submit' class='btn btn-outline-primary' value='$v->product_id' name='edit'>Edit Details</button></form>";
    echo  '</div>';
    echo '</div>';
  }
  echo    '</div>';
